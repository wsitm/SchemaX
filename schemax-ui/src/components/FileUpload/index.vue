...
export default {
  ...
  props: {
    ...
    uploadUrl: {
      type: String,
      default: import.meta.env.VITE_APP_BASE_API + "/common/upload", // 上传文件服务器地址
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    handleBeforeUpload(file) {
      ...
      if (file.name.includes(',')) {
        this.$modal.notifyError('文件名不正确，不能包含英文逗号!');
        return false;
      }
      // 校检文件大小
      if (this.fileSize) {
    ...
    handleUploadSuccess(res, file) {
      if (res.code === 200) {
        ...
        this.number--;
        this.$modal.closeLoading();
        this.$modal.notifyError(res.msg);
