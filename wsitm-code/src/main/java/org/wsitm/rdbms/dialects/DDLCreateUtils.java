...
public class DDLCreateUtils {// NOSONAR
    private static final Log logger = LogFactory.getLog(com.github.drinkjava2.jdialects.DDLCreateUtils.class);
    ...
    private static void transferTableToObjectList(Dialect dialect, TableModel t, List<Object> objectResultList) {
        ...
        // create table
        buf.append(hasPkey ? dialect.ddlFeatures.getCreateTableString() : dialect.ddlFeatures.getCreateMultisetTableString())
        ...
        if (!StrUtils.isEmpty(t.getCheck())) {
            if (features.getSupportsTableCheck())
        ...
        if (t.getTableTail() != null) {//if tableTail!=null, always export tableTail and ignore engine and engineTail
            ...
            String tableTypeString = features.getTableTypeString(); //tableTypeString determined if dialect support engine
        ...
        if (!StrUtils.isEmpty(t.getComment())) {
            if (features.getSupportsCommentOn())
        ...
        // column comment on
        for (ColumnModel c : columns) {
            if (!c.getTransientable() && features.getSupportsCommentOn() && c.getComment() != null
                    && StrUtils.isEmpty(features.getColumnComment()))
    ...
    private static void appendColumnDDLinBuf(Dialect dialect, StringBuilder buf, String tableName, ColumnModel c) {
        ...
        // Identity
        if (GenerationType.IDENTITY.equals(c.getIdGenerationType()) && !features.getSupportsIdentityColumns())
        ...
        if (!StrUtils.isEmpty(c.getColumnDefinition()))
            ...
            if (features.getHasDataTypeInIdentityColumn())
                buf.append(dialect.translateToDDLType(c));
            buf.append(' ');
            if (Type.BIGINT.equals(c.getColumnType()))
                buf.append(features.getIdentityColumnStringBigINT());
            else
                buf.append(features.getIdentityColumnString());
        } else {
            buf.append(dialect.translateToDDLType(c));
            // Default
            ...
            if (!c.getNullable())
                ...
                buf.append(features.getNullColumnString());
        }

        // Check
        if (!StrUtils.isEmpty(c.getCheck())) {
            if (features.getSupportsColumnCheck())
        ...
        if (!StrUtils.isEmpty(c.getComment())) {
            if (StrUtils.isEmpty(features.getColumnComment()) && !features.getSupportsCommentOn())
                ...
                buf.append(StrUtils.replace(features.getColumnComment(), "_COMMENT", c.getComment()));
        }
    ...
    private static void buildSequenceDDL(Dialect dialect, List<String> stringList,
        ...
        for (SequenceIdGenerator seq : notRepeatedSequences) {
            ...
            if (features.getSupportsPooledSequences()) {
                // create sequence _SEQ start with 11 increment by 33
                String pooledSequence = StrUtils.replace(features.getCreatePooledSequenceStrings(), "_SEQ",
                        seq.getSequenceName());
                ...
                String simepleSeq = StrUtils.replace(features.getCreateSequenceStrings(), "_SEQ", seq.getSequenceName());
        ...
        for (TableIdGenerator tg : tbGeneratorList)
            if (tg.getAllocationSize() != 0) {
                ...
                if (!tableExisted.contains(tableName)) {
                    String s = dialect.ddlFeatures.getCreateTableString() + " " + tableName + " (";
                    ...
                    if (!columnExisted.contains(tableAndPKColumn)) {
                        stringList.add("alter table " + tableName + " " + dialect.ddlFeatures.getAddColumnString() + " "
                                + tg.getPkColumnName() + " " + dialect.translateToDDLType(VARCHAR100) + " "
                                + dialect.ddlFeatures.getAddColumnSuffixString());
                        columnExisted.add(tableAndPKColumn);
                    }
    ...
    private static void outputFKeyConstraintDDL(Dialect dialect, List<String> stringList, List<FKeyModel> trueList) {
        if (DDLFeatures.NOT_SUPPORT.equals(dialect.ddlFeatures.getAddForeignKeyConstraintString())) {
        ...
        for (FKeyModel t : trueList) {
            ...
            if (refTableAndColumns.length == 1)
                fkeyTemplate = dialect.ddlFeatures.getAddFKeyRefPkeyString();
            else
                fkeyTemplate = dialect.ddlFeatures.getAddForeignKeyConstraintString();
